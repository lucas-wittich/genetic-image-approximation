...
def run_crossover_experiment(config):
    ...
    os.makedirs("../data/results/crossover", exist_ok=True)
    all_records = []

    for method in crossover_methods:
        print(f"Running crossover method: {method}")
    ...
    df.to_csv("../data/results/crossover/crossover_comparison.csv", index=False)

    plt.figure(figsize=(12, 8))
    for method, group in df.groupby("crossover_method"):
    ...
    plt.title("Crossover Method Comparison")
    plt.xlabel("Generation")
    plt.ylabel("Fitness (↑ = better)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("../data/results/crossover/crossover_comparison.png", dpi=300)
    plt.savefig("../data/results/crossover/crossover_comparison.svg")
    plt.show()


def load_config(defaults):
    parser = argparse.ArgumentParser(description="Compare Crossover Methods in Genetic Algorithm")
...
if __name__ == "__main__":
    ...
    run_crossover_experiment(config)
